...
#define H5FILE_NAME "h5_inconsist.h5"
...
main(int argc, char **argv)
{
    ...
    sprintf(dataset_name, "dataset_%d", mpi_rank * 100);
    if(mpi_rank > 1){
        dset_id =
            H5Dcreate(file_id, dataset_name, H5T_NATIVE_INT, filespace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        H5Dclose(dset_id);
    }
    MPI_Barrier(comm);
    printf("Rank %d finished creating dataset\n", mpi_rank);
