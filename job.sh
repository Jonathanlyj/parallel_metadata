...
if test "x$SLURM_NTASKS_PER_NODE" = x ; then #number of cores per node
   SLURM_NTASKS_PER_NODE=8 # 256 maximum
fi
...
export LD_LIBRARY_PATH="/global/homes/y/yll6162/pnetcdf-source/pnetcdf-install/lib:$LD_LIBRARY_PATH"
srun -n $NP -c 32 --cpu_bind=cores ${SB_EXE} # 8 process per node => 256/8=32
#srun python ...